...
- 2026-08-02 的入口可靠性修订以进程级可取消互斥替代了前项早期的“零排队”描述。磁贴和 Photo Picker 本身各自单飞，因此跨入口同时触发时最多一个有界 Bitmap 任务等待当前推理释放，不会形成无界队列；调用方取消或超时会终止等待并释放像素。
...
- 授权页返回前台时，若系统已报告授权但 listener 尚未连接，App 只请求一次系统管理的 rebind；不自行启动后台服务。平台 `onListenerConnected` 回调在其连接存续期间优先于可能短暂滞后的 enabled-listener 包查询，真实断开回调后再按最新授权结果显示“需授权”或“等待连接”。这只修复健康状态竞态，不等于已经完成上述真实通知 callback 验收。
...
每次新进程启动，App 先显示“纯本地 / 不走网络”的显著声明：当前 Manifest 不声明 Internet 权限，Bill 不向项目方服务器上传通知、截图、账户、余额或账本数据。声明使用 safe drawing insets 与纵向滚动，避免系统栏或大字体遮住确认操作。该声明不能替代第三方 SDK、签名包资源与目标真机审计；工程验证进度和 Experimental 状态只写入仓库文档与发布门，不作为面向用户的宣传句。简中、繁中和英文界面只说明用户当下需要判断的权限范围、识别不确定性与草稿确认路径。设置页只展示当前可用的通知和单次截图/Photo Picker 两类入口；未来只读结果页在实现与验收前不占位。磁贴不可用与无障碍服务详情的设置入口都回到该教程，教程明确图片 OCR 只创建待确认草稿，不证明 provider 已稳定支持。
