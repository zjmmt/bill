...
- 状态：部分实现；用户确认的转账、信用卡还款与退款切片已有自动化证据，其他关系与阈值仍待样本校准
- 所有者：项目维护者
- 最后核验：2026-07-30
- 事实来源：原设计报告的关联思路，经多来源与反例扩展；ADR-0009；[用户确认对账 ExecPlan](../exec-plans/active/0006-user-confirmed-reconciliation.md)；当前领域、应用与 Room 实现
...
当前未实现 `DUPLICATE_OF`、`PENDING_POSTED`、`FUNDED_BY`、`WRAPPED_BY`、充值/提现、余额差异、评分阈值或自动确认。没有真实 provider 样本时不得把本切片宣传为支付宝、微信或银行自动对账。
