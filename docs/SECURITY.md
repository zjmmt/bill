...
| 通知观察去重条目 | Room v10（能力由 v6 引入）；仅安装私有 HMAC 摘要、opaque command/lease、状态和时间 | 已捕获摘要与已失效超过 90 天的活动租约，均只在后续候选回调中一次至多清理 64 条；活动租约 2 分钟后只能复用原 command | 保存原始 `StatusBarNotification.key`、包名、频道、类别、正文或金额；用正文哈希/金额替代系统实例 |
