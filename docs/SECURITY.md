...
| 通知观察去重条目 | Room v11（能力由 v6 引入）；仅安装私有 HMAC 摘要、opaque command/lease、状态和时间 | 已捕获摘要与已失效超过 90 天的活动租约，均只在后续候选回调中一次至多清理 64 条；活动租约 2 分钟后只能复用原 command | 保存原始 `StatusBarNotification.key`、包名、频道、类别、正文或金额；用正文哈希/金额替代系统实例 |
...
### 2026-08-02 v6 侧载候选复核

- 完整 `test lint assembleDebug assembleRelease` 806-task 构建通过；生产边界扫描覆盖 155 个源码文件、26 个依赖文件和 9 个 Manifest，未发现直接日志、遥测依赖或新增高风险权限。
- 内部签名 `6/0.1.0-internal.6` 的双 ABI 原始产物及友好命名副本均由同一仓库证书 pin 验证。arm64 交付包通过 v2 单 signer、16 KiB ZIP 对齐和最终 APK Manifest 检查；仅声明应用自身的签名级动态接收器权限，没有网络、媒体库、短信、悬浮窗或前台服务权限。
- 快捷截图结果反馈只持久化成功/未完成等安全枚举、未读位、过期时间和“可能已存在”标记，不保存截图、OCR 文本、金额、来源、账户或建议 ID。图标、应用标签和验签后的友好文件名不改变 Android 组件、数据流或授权边界。
- 当前闪屏源码加入后，完整 806-task 构建、`git diff --check` 与敏感边界扫描再次通过；扫描覆盖 156 个生产源码文件、26 个依赖文件和 9 个来源 Manifest，仍未发现直接日志、遥测依赖或新增高风险权限。该结论是源码/未签名构建证据，不替代下一签名包的 Manifest 与真机复核。
- 发行脚本只清理 `app/build/outputs/apk/release/` 下四个精确命名的预期产物，并在复制后再次验签原始与友好命名的双 ABI APK；不会递归删除仓库、密钥目录或用户数据。固定 keystore 的离线/异盘备份仍是人工发布门。

### 2026-08-02 v7 截图反馈候选复核
