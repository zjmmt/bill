...
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| 本地账本与存储 | 2 | 3 | 3 | 1 | 1 | Room v11、失败关闭映射与原子仓储已实现；S24U-HK 的最终 55/55 Room suite 已覆盖 v10→v11、余额快照、归档兼容、编辑与 `FUNDED_BY` 确认/撤销/双证据链；仍无数据库应用层加密、真实系统强杀和完整真机矩阵 |
| 受限 CNY/USD 手工账户与期初余额 | 2 | 3 | 3 | 1 | 2 | 现金/钱包仅 CNY，银行卡/信用卡可为 CNY/USD；逐币种平衡、分币种总览、混币失败关闭和 USD 回归已实现，CNY MuMu 冷启持久化及 S24U-HK 的 USD 余额快照 instrumentation 已验证；完整真机矩阵仍待完成 |
...
| CNY 投资持仓纵向切片 | 2 | 3 | 3 | 2 | 1 | 名称/当前金额手填、简繁中及严格英日标签单图 OCR 预填、可选代码/份额/成本、Room v11 原子持仓、既有持仓硬校验与 `INVEST_BUY` 资产平衡均有回归；v7→v11 迁移与 Room 仓储已在 S24U-HK 执行，持仓专用 OCR/UI/通知 callback 尚未真机验收，赎回、估值更新和收益分析未实现 |
| 完整账户、负债、投资 | 2 | 2 | 1 | 2 | 0 | 当前有现金/银行卡/钱包/信用卡、受限 CNY/USD、普通账户余额快照与 CNY 持仓/申购纵向切片；贷款、赎回、估值更新、成本批次、换汇与完整多币种仍未实现 |
