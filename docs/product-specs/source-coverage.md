...
| 通用手工录入/期初余额 | 已实现，发布门未完成 | CNY 路径经自动化与 MuMu API 32 验证；S24U-HK 的 47/47 Room suite 包含 USD 银行/信用卡允许与钱包拒绝，完整 UI/设备矩阵仍未执行 | 现金、电子钱包余额只允许 CNY；银行卡和信用卡允许 CNY/USD。收入/支出先 Draft，选择同币种资金账户后平衡确认；无跨币种汇率或总额 |
