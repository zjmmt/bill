...
| Debug 通知模板采样器 | 已实现研究工具；不属于产品能力 | S24U-HK/API 36 上 10 个采样器合成用例与既有 3 个 route 用例共 13/13 通过；尚未读取本轮真实样本 | 仅 Debug 桌面入口可由用户手动开始/停止/清除；只接受开始之后所选精确包名的新 callback，跨进程无限期恢复，不设自动时间、条数或文件大小限制。仅追加五个有界正文域到 app-private no-backup NDJSON，不读历史/key/actions/RemoteViews，也不生成账本证据。设备内每页倒序展示最多 10 条实际字段及缺失域，并提示 listener 断连、本进程队列丢弃与正文不可读取；系统未交付的事件仍不可推断。Release 无页面、无原始文件名和研究包名；真实样本仍须先导出脱敏并建立回放，不能据此声明支付宝、微信或银行已支持 |
