...
- 状态：部分实现；手工、CNY 投资持仓、通用分享文本、不透明文本证据、本地 CSV/TSV 显式映射、用户显式单次 PNG 收据、受控通知 route catalog/控制面、用户确认对账和持久观察去重已有代码；支付宝、微信与招商银行有 5 条默认关闭的实验性通知 route，生产 catalog/registry 的五路整链内存回放已通过；Quick Settings/Photo Picker 本地 OCR 已有未签名 Release 静态审计、一台设备的合成推理、13 张本机私有真实页面回放、简中合成状态回归、固定内部签名 APK 身份/对齐及 arm64 安装冷启动证据，但通知 callback、真实简中页面和签名 Release 完整资源发布门未完成
- 所有者：项目维护者
- 最后核验：2026-08-02
...
本文件定义目标能力和“什么才算支持”，同时记录生成矩阵上线前的人工核验快照。当前已有受限 CNY/USD 手工账本闭环、CNY 投资持仓的手填/单图 OCR 预填确认，以及 provider-unverified 的显式分享文本、不透明文本文件、用户逐列映射 CSV/TSV 和单次 PNG 收据截图证据/复核闭环；另有 Quick Settings 单次截图与 Photo Picker 最多 5 张串行 OCR。CSV/TSV 只提供来源中立映射，不识别银行或支付平台。PP-OCRv6 small + ONNX Runtime/OpenCV 已静态随包，并通过源码/AAR 与当前未签名 Release 分包的权限、组件、ABI、模型、体积和 16 KiB ZIP 对齐审计；固定内部测试签名双 ABI 包又完成单一证书身份与 ZIP 对齐验证，arm64 正式签名包已在港版 S24 Ultra 覆盖安装并冷启动。该设备上的一组合成中/英/日支付文本已证明模型与原生运行时可加载、推理并释放。同一设备上的 13 张本机私有真实支付过程截图在状态门与空间规则修正后，金额和预期语义均为 13/13；新增的已完成提现页与支付详情页证明 OCR 能读出金额，提现金额丢失来自历史处理中状态的解析优先级而非模型漏字。真实样本覆盖当前繁中/英文页面，简中以四种程序绘制状态页补了真机回归。它们仍不替代真实简中微信页面、系统截图/选图 UI、ELF LOAD 段页兼容、签名 Release 资源和完整目标真机证据。21 条本地真实通知样本经用户授权离线分析后，项目新增支付宝支出/余额收款/基金申购确认、微信付款完成、招商银行快捷支付退款 5 条窄范围通知 route；微信标题允许 `Weixin Pay` 与 `微信支付`，但金额仍只接受已验证的英文正文模板，未知中文正文失败关闭。支付宝基金 route 只接受唯一确认金额和当前零手续费形态，简体来自真实样本，繁体只是兼容别名；它不推断基金标的，用户必须选择既有持仓。所有 route 默认关闭，只用脱敏夹具自动回放，尚未完成真机系统 callback、更新/重启与资源验收。专属文件格式与其他通知事件仍未实现。现金、支付宝余额和微信零钱只能是 CNY；只有银行卡和信用卡可为 USD，且没有汇率换算。电子钱包余额与银行卡是独立资金账户：银行卡流水不能补齐钱包余额内的红包、个人转账、余额消费或余额退款。未来生成器建立后，逐格式实际矩阵必须由代码和测试写入 `docs/generated/`，不得手工伪造。
...
| --- | --- | --- | --- |
| 支付宝通知与文件 | 通知部分实现；文件未实现 | `FALLBACK_REQUIRED`；3 条通知 route 为 `Experimental` 且默认关闭 | 支出、余额收款和基金申购确认各有独立 metadata/content rule、provider parser 与脱敏回放。基金 route 只从“确认成功”形态提取唯一 CNY 确认金额并验证零手续费，提出 `INVEST_BUY`；申请受理、收益/行情、多金额和非零手续费失败关闭，通知不推断标的且必须由用户选择既有持仓。简体格式来自真实样本，繁体字段为兼容别名而非独立样本。扫码成功页无通知、其他通知文案、余额内部事件、文件导入和真机 callback 均未覆盖 |
...
| 通用受控通知 route 证据 | 已实现并承载 5 条实验 route | 不改变三类来源的 `FALLBACK_REQUIRED` | 静态 route catalog、默认关闭的 app-private route 开关、metadata gate、parser 注册和 RawEvent identity 由同一 route 提供；包名 + Android 通知渠道 + 类别（含 null）必须精确匹配。设置页只显示 catalog 安全标签；开启先持久化，关闭先收紧本进程门禁，失败会冻结其他 route 并要求重试或在重启前撤销系统通知使用权。关闭或未知 route 不读正文、不落库；ingress 会再次检查开关。候选使用容量 16 队列、Room v11 HMAC 观察租约和 command 恢复去重；provider parser 在运输后复核 template/version/content，只产出可证明的金额、方向和有限经济事件提示，复核 UI 不显示包名、频道或原文，也不直接过账。生产与回放共同使用同一 parser 清单，五条脱敏夹具已逐条到达 `WAITING_USER` 建议；S24U-HK/API 36 已验证当前 5-route catalog 的默认关闭和 opaque opt-in。内存回放不替代系统 callback，更新/重启与资源证据仍缺失 |
...
| CNY 投资持仓与基金申购复核 | 已实现纵向切片，发布门未完成 | 领域、OCR 标签解析、application、Room schema 和 Compose 已有自动回归；Room v11 迁移/仓储已在 S24U-HK 执行，持仓专用真机 UI/OCR/通知 callback 仍待验证 | 用户手填名称和当前金额，或主动单图本地 OCR 预填后人工确认；代码、份额和成本可选。持仓快照与投资流水分离，基金确认通知只提出 `INVEST_BUY`，必须选择既有持仓与 CNY 资金账户，确认后生成资产间平衡分录；不自动建仓、不更新实时净值、不计普通支出 |
| 用户显式分享 `text/plain` | 已实现，发布门未完成 | 生命周期单测、Room v5 staging 与 MuMu API 32 已验证；当前 schema 为 v10 | 始终为 `GENERIC/SHARE_TEXT`；64 KiB、严格 UTF-8、私有 no-backup 证据和哈希校验；不猜 provider/金额/类型/账户，用户补全；重复只提示；已有逐项两阶段清除、7/30/90/永久保留、已提交与暂存共用的 16 MiB/512 份预算、租约恢复、有界孤儿扫描和 keyset 分页；缺压力、真实系统强杀、自动化 Compose 和完整真机证据 |
...
| 用户显式映射 CSV/TSV | 已实现候选，发布门未完成 | 纯 Kotlin 解析/映射、application 批次、ViewModel 状态机与来源投影测试以及既有全量 JVM/Lint/Debug/Release 构建通过；当前 schema 已升至 Room v11，导入仓储已随 54-test Room 套件在设备执行，真实 SAF UI/文件仍待验收 | 当次 SAF 读取严格 UTF-8，文件最多 2 MiB、5000 数据行、64 列；用户必须映射日期、金额、方向、对方和可选参考号，选择 CNY/USD。有效行各自生成可清除 `GENERIC/STATEMENT_IMPORT` 证据与待复核建议；文件名/URI 不保存，未知表头不自动猜列，拒绝行只留安全错误码，停止后同文件+同映射可续传。行记录使用事务内 O(1) 增量汇总，不再逐行全表扫描。不是任何钱包或银行格式支持声明 |
...
| Quick Settings 单次截图与 Photo Picker 有界多选 OCR | 已实现候选，发布禁止 | 模型哈希/中英日字典、AAR 边界以及未签名 Release 分包的权限、组件、ABI、模型、体积与 16 KiB ZIP 对齐已有静态证据；S24U-HK 合成三语推理 1/1、合成多金额 OCR→v2→建议 1/1、13 张本机私有真实页面金额/预期语义 13/13、简中四状态页定向 app instrumentation 1/1，以及 HardwareBuffer→软件位图→OCR→金额解析真机回归通过。真实磁贴点击已从微信画面建立草稿，两张不同图片已穿过真实 Photo Picker 进入审核。仍缺真实简中微信页面、系统选图取消/失败/批量矩阵、ELF 页兼容和完整目标真机 Release 资源证据 | 磁贴只在用户点击后截取当前画面一次；command 先有 90 秒可取消阶段，在证据准入/容量清理/写入前用 CAS 原子交接取消权与本地提交。handle 注册中不提前宣称取消成功；提交前先释放像素，local commit 另有 15 秒协作式截止，闸门后的超时、取消或非致命异常统一提示“结果未确认”。提交/既成结果不可取消而回调缺失时最多再等一次 15 秒，随后释放并隔离迟到回调。Photo Picker 每批只处理前 1–5 张、逐张串行、显示已处理数量并只汇总一次，活动批次拒绝第二批。磁贴/无障碍设置入口深链到 Bill 教程；无节点读取、手势、后台捕获、相册广泛权限或自动过账。系统截图成功后，硬件位图转换或本地 OCR 失败保存空转录并建立可手填草稿；系统未取得画面才报告截图失败。PP-OCRv6 small 静态随包，ONNX Runtime 建会话前关闭 telemetry；原始像素只作瞬时输入，v2 转录只保留文本和归一化整数边界，v1 可重放。历史待处理步骤只有被空间顺序更晚的精确完成终态覆盖时才解除状态门；完成页仍须显著主金额，正负号仅作用于选中金额。金额和方向始终是 `GENERIC/PHOTO_OCR` 可编辑待复核项。不能据此声称任一 provider 支持 |
...
| 通用 | Quick Settings 单次截图 / Photo Picker 最多 5 张串行 OCR | Experimental，当前发布禁止 | 只在用户点磁贴或选择图片后处理；不绕过 `FLAG_SECURE`，结果必须确认。随包 PP-OCRv6/ONNX 候选已有未签名 Release 静态审计、S24U-HK 合成三语、合成多金额空间链、13 张本机私有真实繁中/英文过程页 13/13、简中四状态页合成回归、硬件截图位图定向回归、真实磁贴到草稿及两张真实 Photo Picker 图片到审核；但尚未完成真实简中微信页面、系统选图取消/失败/批量矩阵、ELF 页兼容和三台目标真机 Release 资源验收，不能用作 provider 支持声明 |
...
2026-08-02 的采集可靠性修正把 `SHARE_FILE` 与 `PHOTO_OCR` 纳入和通知、文本、表格相同的暂存租约事务，避免图片已经写入私有证据后被 Room 误判为 ID 冲突。不同图片可连续建立独立待复核项；相同转录用不同命令再次导入时保留两条不可变证据并标记可能重复，不自动合并。图片取得成功但 OCR 没有可用文字时，保存一个不含虚构金额的空 v2 转录并打开空白待复核项，用户手工补全；它不会产生正式交易。快捷截图的完成/失败结果另保存一个 24 小时内只读一次、且不含截图、OCR 文本、金额、来源、账户或建议 ID 的本地回执，避免 OEM 吞掉后台 Toast 后完全没有反馈。

同日新增真实样本暴露出两个入口差异：相册导入已经识别金额，但历史“处理中”步骤压过了页面下方的完成终态；磁贴截图则在 Android 硬件位图转软件像素时失败。parser 现在只接受空间顺序更晚的精确完成终态解除待处理门，缺少边界时继续保守留空。磁贴先有界缩放硬件位图再复制为软件位图；截图已取得而转换或 OCR 失败时仍建立空白待审核项。Debug 测试使用独立应用 ID，不覆盖或读取内部签名正式版数据；真实原图只保存在 Git 忽略的本机目录，未成为测试夹具或构建资源。
