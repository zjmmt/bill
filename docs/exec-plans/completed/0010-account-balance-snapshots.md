...
- 状态：已完成；Room v11、账户页、自动化、真机、review 与构建证据齐全
- 所有者：项目维护者
- 最后核验：2026-08-01
- 事实来源：MVP G-10、账户/负债/投资规格、对账设计、ADR-0009、ADR-0015、当前 Room v11/账户页实现、自动化与 S24U-HK 真机结果
...
- [x] 2026-08-01 - ExecPlan 0011 已完成、Room v10 已在 S24U-HK 回归，本计划解除排队并开始实施。
- [x] 2026-08-01 - 实现领域、Room v11、应用、账户页和领域/Application/mapper/ViewModel/Room/迁移测试。
- [x] 2026-08-01 - 运行完整构建、仓库守卫与用户指定的 `code-review`；未发现开放 P0/P1，修复归档兼容缺口并补回归。
- [x] 2026-08-01 - 项目负责人明确连接 S24U-HK 后执行最终 `data:local` 55/55、app 16/16，覆盖安装最终 arm64 Debug 并冷启动成功。
...
- `code-review` 发现创建时合法的快照会在账户后来归档后被完整性查询误判为损坏；最终查询保留历史行、停止当前投影且不污染全局状态，并以第 55 条 Room 真机用例固定。
...
- 2026-08-01 - 账户归档只影响当前投影，不改变已保存快照的合法性；不可变历史不能因可变账户展示状态反向失效。
...
已完成。余额快照以独立不可变表保存实际余额、币种、`asOf`、记录时间、可选备注、手工来源和创建命令；创建事务只写快照、审计与幂等回执，不创建交易或 Entries。最新投影按同账户最大 `asOf/recordedAt/id` 选择，并只汇总 `ACTIVE && occurredAt <= asOf` 的同币种分录；信用卡在比较边界转换为正数欠款视角。

验证结果：领域/Application 定向测试通过；S24U-HK 的 Room 55/55、app 16/16 通过；最终 arm64 Debug 覆盖安装并以 `Status: ok` 冷启动。`test lint` 为 571 个 actionable tasks，APK/AndroidTest 构建为 711 个 actionable tasks；仓库守卫、文档检查、生成事实和 `git diff --check` 通过。两份未签名 Release 均通过 16 KiB ZIP 对齐，大小与 SHA-256 记录在 README 与可靠性文档。

审查结果：无开放 P0/P1。归档账户历史快照误判为损坏的问题已修复；超长或含控制字符备注在领域、Application 与仓储边界失败关闭。未读取、提交或上传任何真实通知、截图、账号或 `evidence.local/**` 私有样本。
