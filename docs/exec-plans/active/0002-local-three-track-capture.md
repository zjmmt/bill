...
第一交付实现低功耗通知基础、显著本地声明和单次 PNG 收据回退：默认没有来源模板，因此不会读取任何支付 App 的正文，也不会声称支付宝、微信或银行已接入。它建立以后可由脱敏样本驱动的指定 App 模板安全接入。用户可用 Android 正常截图后经系统 Sharesheet 分享一张 PNG 给 Bill，形成仅本地、无 OCR、无字段猜测的手工复核项。后续磁贴/Photo Picker OCR 属于独立的 [ExecPlan 0004](0004-quick-tile-local-ocr-capture.md)，本地引擎与未签名 Release 静态门已完成但实际推理/真机/签名门未完成；它不改变本计划 Sharesheet PNG 路径的无 OCR 边界。
...
- route 开关原实现先改内存再 `commit()`；启用写盘失败时会在本进程意外放行。修正后开启先持久化，关闭先收紧运行时门禁。另一个容易漏掉的竞态是：关闭写盘失败后若允许修改其他 route，会覆盖唯一的重试提示并可能在重启后恢复旧开启状态；控制面现在会冻结其他开关，直到原隐私写入重试完成。
- S24U-HK 的 instrumentation test context 没有可用的 app data 目录，测试专用 SharedPreferences 写入会失败。Android 回归因此使用 target context 下独立的 `bill.notification-route-enablement.instrumentation-test` 文件，并且只删除该测试文件；它不触碰生产偏好名。测试中断最多遗留无功能的 fixture ID，不会改变生产 route 状态。
- Android 设置中的 enabled-listener 包集合只证明系统授权存在，不证明 `NotificationListenerService` 已连接。健康状态现在另行跟踪 `onListenerConnected` / `onListenerDisconnected`；One UI 或 HyperOS 断连时不能显示“已准备好”。
...
- 2026-07-31 - 通知 route 控制面只接收 catalog 导出的 opaque route ID 与安全标签；磁盘写入在串行后台边界完成。开启必须先 `commit()` 成功再放行运行时门禁；关闭先立即收紧本进程门禁，再尝试持久化，失败时保持关闭、冻结其他开关并显式要求重试。该关闭保证严格限定于当前进程：若持久介质不可写，用户必须在退出或重启前重试，仍失败则到 Android 设置撤销 Bill 的应用级通知使用权。健康状态还要求 listener 实际连接后才能显示就绪。系统设置入口只在存在已开启 route、或已有系统授权需要管理/撤销时显示；无路线且无既有授权时不诱导授权。
...
- 保留风险：当时首个非空 catalog 前仍须补只显示安全标签的 Bill 内 route 选择器、“有目录但未开启”健康状态，以及 SharedPreferences 跨实例/旧 route ID 的 Android 回归；这些控制面缺口已由 2026-07-31 第六轮关闭。真实 callback/OEM 行为和资源基线仍需另行明确真机授权。

## 第六轮代码审查记录（2026-07-31）

- 范围：route 设置控制器、SharedPreferences 持久边界、最小权限 CTA、listener 连接健康、TalkBack 语义、Android instrumentation 隔离和生产 fixture 隔离。
- 已修复：开启/关闭写入采用不同的失败关闭顺序；失败态冻结无关 route 并保留精确重试；同值请求不重复写盘；损坏/旧偏好默认全关；设置页只接收安全标签，整行合并为有标签的 TalkBack 开关；系统设置入口失败会提示手工路径；已有系统授权时保留管理/撤销入口；授权存在但 listener 未连接时不再显示就绪。AndroidTest 改用独立测试偏好文件，Release 静态检查未发现 fixture route、包名或测试类。
...
9. 当前控制面切片：以 catalog presentation 建立只读设置快照，通过单飞串行后台命令启停 route；开关不可越过 catalog，开启不得在持久化失败时放行，关闭失败则只保证本进程立即收紧并要求在重启前重试或撤销系统授权。设置页仅在 route 已开启时提供授权入口；若应用级系统授权已存在，则无论 route 是否暂停或 catalog 是否为空都保留管理/撤销入口。健康状态还要求 listener 实际连接后才能显示就绪。fixture catalog、损坏偏好和跨实例 Android 回归覆盖控制面；生产 catalog 仍为空。
...
尚未完成。当前已交付空模板通知证据基础、Room v7 中保留的 v6 持久观察去重、有界回调队列、安全标签 route 控制面、最小权限授权/撤权入口、listener 连接健康、来源待复核展示、显著本地/权限说明，以及单次 PNG 收据的本地手工复核回退。S24U-HK 只验证了 3 个隔离偏好 instrumentation 和 Bill 冷启动，不包含通知授权、系统 callback、真实 provider 或资源数据。磁贴/Photo Picker OCR 已由 ExecPlan 0004 接入 PP-OCRv6 并通过本地无网络静态门，但实际推理、签名发行和目标真机门仍未通过。完成时在此记录真实模板、设备回放、测得资源结果、未开放能力和对来源支持标签的影响。
