...
- 状态：进行中；本地 OCR 引擎、未签名 Release 静态审计、固定内部签名 APK 身份/对齐、港版 S24 Ultra 合成三语/简中状态回归、13 张本机私有真实页面回放、Photo Picker 成功路径与真实磁贴到草稿已完成，真实简中微信页面、签名包资源与完整目标真机发布门未完成
- 所有者：项目维护者
- 最后核验：2026-08-02
...
当前代码已经实现磁贴、单次截图、最多 5 张 Photo Picker 串行导入、转录证据和 Draft 链。OCR 已从 Chinese ML Kit 替换为随包的 PP-OCRv6 small ONNX 检测/统一多语言识别模型，运行时为 ONNX Runtime Android 1.24.3 与 OpenCV Android 4.12.0；模型文件合计 31,190,469 字节，来源与 SHA-256 固定在 `ocr/paddle/MODEL_PROVENANCE.md`。引擎无后台入口，共用一个进程级可取消推理槽、两条 CPU 线程、识别 batch 为 1、输入最长边 1600，且每次任务结束释放会话；磁贴和 Photo Picker 各自单飞，跨入口最多一个有界任务等待。依赖 AAR 与当前 arm64-v8a/x86_64 `release-unsigned` 分包的静态审计未发现网络权限、传输组件、Job 或 Alarm；代码在建会话前调用 `setTelemetry(false)`。分包权限、组件、ABI、模型、体积和 16 KiB ZIP 对齐已核验，固定内部测试签名的双 ABI 包也已完成证书身份/ZIP 对齐验证，arm64 正式签名包已在 S24U-HK 覆盖安装并冷启动。港版 S24 Ultra 的合成三语模型、合成多金额空间链、13 张本机私有真实繁中/英文过程页、真实 Photo Picker 成功路径、真实磁贴到草稿和简中四状态页 instrumentation 已通过；但真实简中微信页面、选图取消/失败/批量矩阵、ELF LOAD 段页兼容、签名 Release 资源和三台目标真机验收尚未完成，因此仍不能标为发布级支持。
...
- [x] 2026-08-02 - 新增两张真实微信支付详情/提现完成页，修复历史处理中状态和 HardwareBuffer 软件位图转换后，私有回放总计 13/13；v5 在 S24U-HK 的真实磁贴点击成功建立草稿，两张不同 Photo Picker 图片也均进入审核。
- [x] 2026-08-02 - v6 已由本机可见 CMD 隐藏输入密码生成；双 ABI 原始/友好命名副本通过固定证书、16 KiB 对齐、最终权限、本地化标签和图标检查。
- [x] 2026-08-02 - v6 已在本轮明确连接的 S24U-HK 以 `adb install -r` 保留数据覆盖安装；设备内版本 `6/0.1.0-internal.6`、非 Debug，冷启动后进程存在。
- [x] 2026-08-02 - v6 真机证明磁贴副标题和返回 Bill 后提示不能提供截图当刻反馈；v7 改为在平台成功取得画面后显示 180 ms、无内容且不可触摸/聚焦的当前页闪屏。固定签名 v7 保留数据覆盖安装后，项目负责人在外部页面目视确认闪屏可见。
- [ ] 2026-08-02 - 复测磁贴即时提示、持久结果副标题、无关页面空白草稿、安装图标和本地化名称。
...
- 已修复：OCR 最初从可排队互斥改为零排队单任务；2026-08-02 的入口可靠性修订又将其收敛为一个进程级可取消互斥槽。磁贴和 Photo Picker 本身各自单飞，因此跨入口最多只有另一个有界 Bitmap 任务等待，不会形成无界队列，也不会把短暂重叠直接当模型失败。检测和识别模型顺序加载，模型字节、输入 tensor 与输出概率在所有路径擦除；创建与释放在取消状态下仍安全完成；HardwareBuffer、Bitmap、Mat 和裁剪中间量具有单一所有者并在异常/取消时释放；选图、分享和 OCR 的临时字节在 prompt cancellation 前后均擦除。输入由 6 MP/2048×4096 收紧为 2.56 MP/1600×1600；TileService 的旧 API Lint 抑制移到精确方法；截图 API 30 类型只在版本门内构造；arm64-v8a 与 x86_64 使用分包，删除 32 位 ABI，避免一个 APK 携带四套原生库。
...
3. 已完成代码与静态包接入：`ocr:paddle` 保存经 Apache-2.0 审查的 PaddleOCR Android 源码与静态模型；运行时显式关闭遥测，固定两条 CPU 线程、batch 1、单槽可取消推理并在成功、失败和取消时释放会话、Bitmap 与临时字节。arm64-v8a/x86_64 目标 ABI 与未签名分包体积已构建测量，固定签名身份、对齐及 arm64 安装冷启动已验证，签名 Release 资源仍待验。
4. 部分完成：构建任务固定模型 SHA-256 并检查中/英/日字典；港版 S24 Ultra 已运行三语合成支付文本 1/1、合成多金额空间链 1/1、13 张本机私有真实繁中/英文过程页 13/13、真实磁贴到草稿、两张真实 Photo Picker 图片到审核与简中四状态页 1/1。真实简中微信页面、选图完整失败矩阵与 Release 资源数据仍未完成。
5. 已完成但需复验：进程内单并发 command controller、`TileService` 和专用 AccessibilityService；断连、忙碌、API 不支持、系统拒绝、OCR/入库失败均更新磁贴/Toast 安全状态。One UI 真机证明这些延迟/折叠通道不能作为截图当刻反馈后，service 又在平台截图成功回调之后增加 180 ms、无内容、不可触摸/不可聚焦的 `TYPE_ACCESSIBILITY_OVERLAY` 闪屏；它不代表 OCR 或入库完成。每个活动 command 先有 90 秒可取消阶段；本地提交自身有 15 秒协作式截止，提交/既成结果不可取消时最多增加一次 15 秒收尾宽限，随后以“结果未确认”释放并用 opaque request identity 丢弃迟到回调。设置相关入口深链到 Bill 教程。
6. 已完成：Photo Picker 最多返回 5 张，ViewModel 以单飞批次串行逐张调用同一单槽 OCR engine；设置页显示处理进度，第二批在活动批次结束前被拒绝，结束时只汇总一次并打开首个可复核项。每张仍受 16 MiB 文件、32 MP 来源和 2.56 MP OCR 输入上限，不取得整库权限。
...
交互、来源链、有界多选、本地引擎替换、空间转录 v2、v1 重放、全量静态构建、未签名 Release 审计与固定签名双 ABI 身份/对齐已实现；设置深链、90 秒截图租约/迟到回调隔离、handle 注册竞态、15 秒本地提交截止/提交后未知态、Photo Picker 单飞批次与一次汇总，以及可滚动 safe-area 启动声明和统一中文披露均已纳入验证。v6 的真实磁贴链能建立草稿但截图当刻无可见反馈；v7 仅在平台成功取得画面后于外部当前页短暂闪屏，并已由项目负责人在 S24U-HK 目视确认。港版 S24 Ultra 的合成三语模型与合成多金额空间链 instrumentation 已通过，但真实简中/日文页面、完整系统截图/选图 UI、ELF 页兼容和其余目标真机证据缺失仍使整体 OCR 能力不具备发布资格。设备轮继续记录三台真机单次耗时/峰值/空闲释放、电量、失败矩阵和仍未达到 provider 支持的范围。
