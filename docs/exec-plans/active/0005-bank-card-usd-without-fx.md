...
- 对支付宝/微信候选强制 CNY，银行来源保留未来可验证的 USD 契约。
...
- [x] 2026-07-26 - 将外部来源草稿币种与资金账户绑定；支付宝/微信/通用路径固定 CNY，银行保留明确币种入口。
- [x] 2026-07-26 - 补 JVM、application、Room integrity 和 Android test APK 编译回归；混币分录、USD 钱包和来源 USD 绕过均失败关闭。
- [ ] 运行完整构建、文档检查和最终 `code-review`；Android instrumentation 与真机仍不能由 APK 编译代替。
...
6. 为支付宝/微信候选保留 CNY-only 断言，为未来银行 USD 实现公开的“需明确币种证据”门；未知格式只进入诊断/待复核。
...
- 2026-07-26 - 来源仓储原本已在最终写入时拒绝支付宝/微信/通用 USD；本轮将同一策略放到来源契约、application 和复核 UI。异常候选不会显示美元输入框，也不会等到落库才返回笼统失败。
