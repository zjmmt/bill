...
- 状态：部分实现；本地账本、CNY 投资持仓、通用显式文本/CSV/TSV 证据、用户确认对账、受控通知边界、5 条默认关闭的 provider 实验 route、route 控制面、持久观察去重与实验性随包 OCR 已实现；既有 Room、合成 OCR 与 11 张本机私有真实页面回放已有 S24U-HK 证据，新增持仓/基金 route 的真机验收、通知 callback、真实简中页面和完整资源发布门待验证
- 所有者：项目维护者
- 最后核验：2026-08-01
...
通知 route 控制面只接收静态 catalog 导出的 opaque ID 与安全标签。开启先同步持久化再放行运行时门禁；关闭先通过 volatile 不可变快照收紧本进程门禁，再持久化并在失败时冻结其他开关。失败关闭的跨进程边界会明确披露：若重试仍失败，用户须在退出或重启前撤销 Android 的应用级通知使用权。授权 CTA 只在已启用 route 需要授权，或系统授权仍在需要管理/撤销时出现；健康状态把系统授权与 listener 实际连接分开。当前生产 catalog 含 5 条默认关闭的实验 route；metadata 与正文模板双重通过后，provider parser 会复核 envelope 身份并只生成可证明的 CNY 金额、方向和有限经济事件提示。微信 route 的标题允许 `Weixin Pay` 与 `微信支付` 两个本地化别名，稳定 route ID 不变；金额仍只来自已验证的正文模板，未经真实样本验证的中文正文失败关闭。支付宝基金申购确认 route 只接受唯一确认金额和零手续费形态，提出 `INVEST_BUY` 后要求用户选择既有持仓，不从通知推断标的；繁体字段只是失败关闭的兼容别名。S24U-HK/API 36 的 app instrumentation 只验证过此前 4-route catalog 的隔离偏好、默认关闭和 opaque opt-in；新增 route 尚未覆盖系统 callback、provider 语义或资源。

以上是代码实现状态，不是发布支持结论。2026-08-01 持仓变更后的 `test lint` 成功（569 个 actionable tasks：30 executed、539 up-to-date），随后 `assembleDebug assembleRelease :data:local:assembleDebugAndroidTest :ocr:paddle:assembleDebugAndroidTest :app:assembleDebugAndroidTest` 成功（711 个 actionable tasks：77 executed、1 from cache、633 up-to-date），覆盖全部 JVM、Lint、Debug/未签名 Release 分包和三组 AndroidTest APK 编译。此前 arm64-v8a/x86_64 分包已通过 16 KiB ZIP 对齐；本轮没有把 APK 编译冒充真机执行。S24U-HK/API 36 的 app 14/14 只覆盖 Debug 采样、偏好隔离和当时 4-route catalog 默认关闭；没有系统 callback。该设备的 `data:local` 47/47 已覆盖 v1→v7、结构化导入/对账/通知观察/证据仓储，随包 OCR 合成三语 1/1 证明模型和原生运行时可加载、推理与释放；另有合成多金额空间链 1/1、本机私有真实繁中/英文过程页 11/11 与简中四状态页 1/1。真实简中微信页面、系统截图/选图 UI、Release 资源、签名发行、ELF 页兼容、新 route callback/更新/重启、投资、自动化 UI/系统强杀和完整真机矩阵不在已验证断面内。
