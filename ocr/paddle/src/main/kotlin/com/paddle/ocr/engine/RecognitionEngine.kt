...
class RecognitionEngine(
    ...
    fun recognize(crops: List<Mat>): RecognitionResult {
        ...
        return try {
            ...
            val inference = ortManager.runRecognition(preResult.tensorData, preResult.shape)
            outputData = inference.first
            val outputShape = inference.second
            val inferenceMs = System.currentTimeMillis() - infStart

            // Postprocess (CTC decode)
            val postStart = System.currentTimeMillis()
            val decoded = CTCDecoder.decode(inference.first, outputShape, characterList)
