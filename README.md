...
当前仓库已经从“文档先行”进入多条纵向实现：受限 CNY/USD 手工账户/期初余额/收入支出账本、Android Sharesheet 与 SAF 的通用显式文本证据、用户逐列映射的本地 CSV/TSV 行导入、转账/信用卡还款/退款的用户确认对账、系统 Sharesheet 单次 PNG 收据证据、默认空目录的通知安全边界，以及用户显式触发的截图/Photo Picker OCR。账本、文本、导入与安全边界已有自动化；CNY 路径还有 MuMu API 32 证据，港版 S24 Ultra 另有应用、Room 与随包 OCR 的合成 instrumentation。OCR 已换成静态随包的 PP-OCRv6 small + ONNX Runtime/OpenCV，本地代码、依赖 AAR 和当前 `release-unsigned` 分包的静态审计未发现联网入口，港版 S24 Ultra 也已用随包模型通过一组中/英/日合成识别；真实支付截图准确率、目标真机资源验收、签名发行包，以及真实支付宝、微信支付和具体银行的完整适配仍未完成。
...
- 截图/Photo Picker OCR：下拉快捷设置的“截图记账”只在用户点击后截一帧；单次 command 先有 90 秒可取消阶段，截图、识别或本地提交尚未开始时会真正取消。在证据准入、容量清理和写入发生前，以一次原子 CAS 把取消权交给设有 15 秒协作式截止、输入有界且无网络的本地提交；进入提交前先释放 HardwareBuffer/Bitmap。cancellation handle 尚在注册时不会把 `null` 误当作取消成功；提交闸门之后的超时、取消或非致命异常一律保守视为“结果未确认”，最多再等一次 15 秒收尾宽限并提示重试前先查待复核。opaque request identity 会丢弃更晚的旧回调。系统 Photo Picker 每批只处理前 1–5 张，始终逐张串行，显示已处理数量并只给一次结果汇总；活动批次期间不接受第二批。磁贴不可用入口和无障碍服务详情页会深链到 Bill 的设置教程。两条路径都不读取整本相册、不轮询屏幕、不执行手势或自动点击，结果只生成可编辑 Draft。PP-OCRv6 small 的中英日统一模型静态随包，运行时零排队、两条 CPU 线程、batch 1、最长边 1600，并在每次任务后释放会话。空间转录 v2 只额外保存每行 0..10000 归一化整数边界，不保存原始像素；解析器继续兼容 v1 文本证据，并且只在独立主金额相对正文和其他独立金额明显更高时预填，多金额等大或缺少布局证据仍保持空白。港版 S24 Ultra 已通过随包模型的一组合成中/英/日识别，以及一条程序绘制多金额图片经随包 OCR、空间转录 v2 到主金额建议的定向 app instrumentation。真实支付截图、系统截图/选图 UI、ELF LOAD 段页兼容、峰值资源和三台目标真机仍未验收，因此它仍是 Experimental，不是已支持的支付来源或可分发发行包。
...
- 受控通知路由与首批实验规则：`NotificationListenerService` 先按包名、具体 Android 通知渠道和类别精确门禁；静态 route catalog、默认关闭的本地 route 开关、parser 注册和 RawEvent 来源身份共用同一条 route。生产目录现有 4 条本地随包、默认关闭的实验规则：支付宝支出、支付宝余额收款、微信英文付款完成和招商银行快捷支付退款。用户须逐条显式开启，ingress 会在落库前再次检查开关；开启必须先写盘成功，关闭先收紧本进程门禁。命中规则只提取一个 CNY 金额和明确方向，进入 `RawEvent -> ParseAttempt -> source proposal -> Draft` 待复核链，不猜商户、资金账户或直接入账。元数据相同但正文不匹配的通知会在内存中丢弃；其中微信支付通知与普通微信消息可能共用频道，开启该规则意味着 Bill 会在本机读取同频道通知的有界正文再过滤，设置标签会显式提示这一点。候选仍使用容量 16 的单消费者队列与 Room v7 的安装私有 HMAC 观察租约。代码只用脱敏夹具回放，尚未完成系统 callback、更新/重启、OEM 后台和资源真机验收，因此不是支付宝、微信或招商银行的正式支持声明。
...
- 验证：2026-07-31 最新完整 `test lint assembleDebug assembleRelease :data:local:assembleDebugAndroidTest :ocr:paddle:assembleDebugAndroidTest :app:assembleDebugAndroidTest` 成功（891 个 actionable tasks：71 executed、820 up-to-date），覆盖全部 JVM 测试、Lint、Debug/未签名 Release 分包和三组 AndroidTest APK 编译；空间 OCR 审查修复后的 `generic-photo-ocr`、application 与 app 单测又强制重跑 167 个任务全部通过。arm64-v8a Release 为 90,597,641 bytes（SHA-256 `5c6e58f02a38aef142130fc723983a7ce445020334a0aee04392e8b3d9a2bae1`），x86_64 Release 为 128,419,330 bytes（SHA-256 `881ddc76dcebf7725d003a54fe3813ad2ff9a6d651dee302882327696b63ef0c`）；两包通过 16 KiB ZIP 对齐。MuMu API 32 保留此前 32 个 v1→v5 Room/证据 instrumentation 结果；港版 S24 Ultra/API 36 本轮另有 app 14/14、`data:local` 47/47、随包 OCR 合成三语 1/1，以及程序绘制多金额图片的空间 OCR app 定向 instrumentation 1/1 通过。上述设备结果没有运行新 provider route 的系统 callback、真实支付截图、系统截图/选图 UI、OEM 后台或资源测试，不构成 `NLS-01` 或 provider 发布证据。完整构建证据见 [可靠性](docs/RELIABILITY.md)，设备结果与未测项见 [Android 设备兼容与真机验收](docs/design-docs/android-device-compatibility.md)。
