...
- 受控通知路由与首批实验规则：`NotificationListenerService` 先按包名、具体 Android 通知渠道和类别精确门禁；静态 route catalog、默认关闭的本地 route 开关、parser 注册和 RawEvent 来源身份共用同一条 route。生产目录现有 4 条本地随包、默认关闭的实验规则：支付宝支出、支付宝余额收款、微信付款完成和招商银行快捷支付退款。用户须逐条显式开启，ingress 会在落库前再次检查开关；开启必须先写盘成功，关闭先收紧本进程门禁。微信 route 把通知标题 `Weixin Pay` 与 `微信支付` 视为同一来源提示，但当前只有真实样本对应的英文 `金额 + paid` 正文模板通过；未经样本验证的简中/繁中正文继续拒绝，标题本身不作为金额。命中规则只提取正文内一个 CNY 金额和明确方向，进入 `RawEvent -> ParseAttempt -> source proposal -> Draft` 待复核链，不猜商户、资金账户或直接入账。元数据相同但正文不匹配的通知会在内存中丢弃；其中微信支付通知与普通微信消息可能共用频道，开启该规则意味着 Bill 会在本机读取同频道通知的有界正文再过滤，设置标签会显式提示这一点。候选仍使用容量 16 的单消费者队列与 Room v7 的安装私有 HMAC 观察租约。代码只用脱敏夹具回放，尚未完成系统 callback、中文正文、更新/重启、OEM 后台和资源真机验收，因此不是支付宝、微信或招商银行的正式支持声明。
- Debug 模板研究入口：开发包另有独立桌面入口，由用户选择精确包名并手动开始、停止和清除。它只追加开始之后的新 callback 到 app-private no-backup 文件，跨进程保持活动，不设自动时间、条数或文件大小限制；不读历史通知、notification key、actions、RemoteViews，也不创建账本记录。页面每次倒序读取最多 10 条，逐项展示来源、时间、channel/category 与五个允许正文域，明确标出未提供字段，并显示当前 listener 状态及本进程可确认的队列/正文漏记；系统从未交付的通知无法反推出内容或精确数量。21 条真实通知样本与 11 张过程截图已按用户要求只保存在两份 Git 忽略的本机私有副本，设备端原始数据也未删除；离线安全盘点只将 5 条样本归入上述 4 个候选规则，其余 16 条保持拒绝。Release 包没有采样页面、原始文件名、研究包名或研究文案。
...
- 外部来源：支付宝、微信支付和招商银行现各有范围很窄的实验性通知适配器与脱敏回放，但只覆盖上述 4 种模板；专属文件适配器、完整事件覆盖、真机系统 callback 回放和发布证据仍未完成，因此三类来源当前状态仍统一为 `FALLBACK_REQUIRED`。通用文本/CSV/TSV 入口只证明 Bill 能处理用户明确选择并映射的不可信文件，不证明来源 App。银行卡只覆盖银行卡/信用卡资金腿，不能替代微信零钱或支付宝余额内部流水。
