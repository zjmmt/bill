...
- 验证：2026-07-31 最新完整 `test lint assembleDebug assembleRelease :data:local:assembleDebugAndroidTest :ocr:paddle:assembleDebugAndroidTest :app:assembleDebugAndroidTest` 成功（891 个 actionable tasks：80 executed、811 up-to-date），覆盖全部 JVM 测试、Lint、Debug/未签名 Release 分包和三组 AndroidTest APK 编译；其中新增 provider 定向套件与 application 纵向测试已强制重跑 173/173 通过。arm64-v8a Release 为 90,597,641 bytes（SHA-256 `d76a2c91013bc2991cf8e61d71e52a0083edf33a6bb8ac8b74c253918cfad74e`），x86_64 Release 为 128,419,330 bytes（SHA-256 `62ec182dfed546540d68536458de181abe55e99ff32cab6dd63da7f2b44c1e4e`）；两包通过 16 KiB ZIP 对齐。MuMu API 32 仍只有此前 32 个 v1→v5 Room/证据 instrumentation 结果；港版 S24 Ultra 于 Android 16/API 36 上运行过 13 个通知控制/采样合成 instrumentation 并全部通过，其中 10 个覆盖 Debug 采样持久化、停止边界和设备内分页预览，3 个覆盖 route 偏好隔离。本次最新完整构建没有连接设备；用户重新插线后 ADB 仍未枚举到设备，这些既有用例没有运行新 provider route 的系统 callback，不构成 `NLS-01`、OEM 后台或 provider 发布证据。完整构建证据见 [可靠性](docs/RELIABILITY.md)，设备结果与未测项见 [Android 设备兼容与真机验收](docs/design-docs/android-device-compatibility.md)。
