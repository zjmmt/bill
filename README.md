...
- 受控通知路由基础：`NotificationListenerService` 先按包名、具体 Android 通知渠道和类别精确门禁；静态 route catalog、默认关闭的本地 route 开关、parser 注册和 RawEvent 来源身份共用同一条 route。生产目录为空时连通知正文都不读取；未来 route 即使进入目录，也只以本地安全标签展示，须用户显式开启，且 ingress 会再次检查开关，避免已经排队的内容在关闭后落库。开启必须先写盘成功才放行；关闭先收紧本进程门禁，写盘失败会冻结其他开关并要求立即重试，若仍失败则提示在退出/重启前到 Android 设置撤销 Bill 的应用级通知使用权。设置入口只在已有启用 route 需要授权、或系统授权仍在需要管理/撤销时出现；只有系统授权且 listener 实际连接后才显示就绪。候选只会进入容量 16 的单消费者队列；当前 Room v7 延续 v6 引入的安装私有 HMAC 摘要和租约，用同一 command 避免通知更新/进程恢复重复创建 RawEvent。合成信封最多 8 KiB，只进入来源待复核链，不猜金额、账户或 provider，也不自动入账。港版 S24 Ultra/API 36 已通过 3 个隔离 route 偏好 instrumentation，但没有授予通知使用权、运行真实 callback 或读取真实内容。这不是支付宝、微信或银行支持声明。
...
- 验证：2026-07-31 完整 `test lint assembleDebug assembleRelease :data:local:assembleDebugAndroidTest :ocr:paddle:assembleDebugAndroidTest :app:assembleDebugAndroidTest` 成功（879 个 actionable tasks），覆盖全部 JVM 测试、Lint、Debug/未签名 Release 分包和三组 AndroidTest APK 编译，已包含通知 route 失败关闭、最小权限 CTA、listener 连接健康与 TalkBack 开关修复。arm64-v8a Release 为 90,581,257 bytes（SHA-256 `9e3c33d7a1a5f127a1f165f99102dc6981fa82d9a1d57767d48579dee2fa0b3e`），x86_64 Release 为 128,402,946 bytes（SHA-256 `d924e1be714920ae6bbfd5fa8d00d67cf9d26fed6b6fda1317aa69c60d5f3cfe`）；两包重新通过 16 KiB ZIP 对齐。MuMu API 32 仍只有此前 32 个 v1→v5 Room/证据 instrumentation 结果；港版 S24 Ultra 除既有合成应用级冒烟外，已于 2026-07-31 在 Android 16/API 36 上通过 3 个隔离 route SharedPreferences instrumentation（跨实例启停、旧 ID 丢弃、损坏类型失败关闭），并在最终审查修复后对当前源码再次 3/3 通过。这些用例没有授予通知使用权、运行 listener callback 或读取真实内容，不构成 `NLS-01`、OEM 后台或 provider 证据。完整构建证据见 [可靠性](docs/RELIABILITY.md)，设备结果与未测项见 [Android 设备兼容与真机验收](docs/design-docs/android-device-compatibility.md)。
