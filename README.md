...
- 状态：部分实现；受限 CNY/USD 手工账本、不可变余额快照、完整可编辑审核、通用显式证据链、本地 CSV/TSV 映射、用户确认对账、5 条实验通知 route、随包本地 OCR 与固定内部测试签名包已落地，arm64 正式签名包已在 S24U-HK 覆盖安装并冷启动；真实生产 callback、签名 Release 资源、完整设备矩阵与密钥备份仍待完成
- 所有者：项目维护者
- 最后核验：2026-08-02
...
当前仓库已经从“文档先行”进入多条纵向实现：受限 CNY/USD 手工账户/期初余额/收入支出账本、Android Sharesheet 与 SAF 的通用显式文本证据、用户逐列映射的本地 CSV/TSV 行导入、转账/信用卡还款/退款的用户确认对账、系统 Sharesheet 单次 PNG 收据证据、5 条默认关闭的生产通知 route，以及用户显式触发的截图/Photo Picker OCR。账本、文本、导入与安全边界已有自动化；CNY 路径还有 MuMu API 32 证据，港版 S24 Ultra 另有应用、Room 与随包 OCR instrumentation。OCR 已换成静态随包的 PP-OCRv6 small + ONNX Runtime/OpenCV，本地代码、依赖 AAR 和当前 `release-unsigned` 分包的静态审计未发现联网入口；同一设备上的 11 张本机私有真实支付过程截图经状态门和空间规则修正后，金额与预期语义均为 11/11，另有简体中文四状态页的程序绘制真机回归。原始截图、OCR 文本和通知正文不进入 Git。仓库已提供本机隐藏输入密码、仓库外固定内部测试 keystore、单次 Gradle 进程和双 ABI `apksigner` 验证的侧载路径；首个固定身份包已在本机完成，随后 `3/0.1.0-internal.3` arm64 APK 已在 S24U-HK 覆盖安装，版本、非调试标志和冷启动通过。密钥备份仍须由维护者收口。真实简中微信页面、目标真机完整资源，以及 5 条 route 的真实系统 callback/OEM 存活仍未完成。
...
- 截图/Photo Picker OCR：下拉快捷设置的“截图记账”只在用户点击后截一帧；单次 command 先有 90 秒可取消阶段，截图、识别或本地提交尚未开始时会真正取消。在证据准入、容量清理和写入发生前，以一次原子 CAS 把取消权交给设有 15 秒协作式截止、输入有界且无网络的本地提交；进入提交前先释放 HardwareBuffer/Bitmap。cancellation handle 尚在注册时不会把 `null` 误当作取消成功；提交闸门之后的超时、取消或非致命异常一律保守视为“结果未确认”，最多再等一次 15 秒收尾宽限并提示重试前先查待复核。opaque request identity 会丢弃更晚的旧回调。系统 Photo Picker 每批只处理前 1–5 张，始终逐张串行，显示已处理数量并只给一次结果汇总；活动批次期间不接受第二批。磁贴不可用入口和无障碍服务详情页会深链到 Bill 的设置教程。两条路径都不读取整本相册、不轮询屏幕、不执行手势或自动点击，结果只生成可编辑 Draft。PP-OCRv6 small 的中英日统一模型静态随包，运行时零排队、两条 CPU 线程、batch 1、最长边 1600，并在每次任务后释放会话。空间转录 v2 只额外保存每行 0..10000 归一化整数边界，不保存原始像素；解析器继续兼容 v1 文本证据。失败、取消、拒绝、未领取/未打开、处理中、预计到账或待完成状态优先于金额版面，整页不预填；只有明确完成上下文和显著主金额同时成立时才允许从 OCR 拆开的纯小数行恢复金额，正负号也只作用于被选中的主金额且要求交易详情上下文。普通付款页的促销红包文案不会把已完成支出误判成红包资金事件；真正的红包、转账、退款、充值和提现仍不猜普通收支方向。港版 S24 Ultra 的 11 张本机私有真实过程截图修正前完整语义 7/11，修正后金额与预期语义均为 11/11；单张 OCR 约 1.297–2.026 秒，中位数 1.688 秒。该 Debug instrumentation 进程观察到的 PSS 只作资源线索，不是 Release 峰值结论。真实样本覆盖当前繁中/英文页面；简中规则另以四张程序绘制状态页在真机 1/1 通过，但真实简中微信页面、系统截图/选图 UI、ELF LOAD 段页兼容、Release 资源和三台目标真机仍未验收，因此它仍是 Experimental，不是已支持的支付来源或可分发发行包。
- OCR 并发边界修订（2026-08-02）：上项早期的“零排队”描述已由当前实现取代。磁贴与 Photo Picker 各自单飞并共用一个进程级 OCR 槽；两入口同时触发时最多只有另一入口的一个有界任务可取消地等待，不形成无界 Bitmap 队列。这样避免短暂重叠被直接当作模型失败，同时仍限制为单次模型推理。
...
- 受控通知路由与首批实验规则：`NotificationListenerService` 先按包名、具体 Android 通知渠道和类别精确门禁；静态 route catalog、默认关闭的本地 route 开关、parser 注册和 RawEvent 来源身份共用同一条 route。生产目录现有 5 条本地随包、默认关闭的实验规则：支付宝支出、支付宝余额收款、支付宝基金申购确认、微信付款完成和招商银行快捷支付退款。用户须逐条显式开启，ingress 会在落库前再次检查开关；开启必须先写盘成功，关闭先收紧本进程门禁。微信 route 把通知标题 `Weixin Pay` 与 `微信支付` 视为同一来源提示，但当前只有真实样本对应的英文 `金额 + paid` 正文模板通过；未经样本验证的简中/繁中正文继续拒绝，标题本身不作为金额。命中规则只提取正文内一个 CNY 金额和明确方向或有限基金事件，进入 `RawEvent -> ParseAttempt -> source proposal -> Draft` 待复核链，不猜商户、资金账户、基金标的或直接入账。元数据相同但正文不匹配的通知会在内存中丢弃；其中微信支付通知与普通微信消息可能共用频道，开启该规则意味着 Bill 会在本机读取同频道通知的有界正文再过滤，设置标签会显式提示这一点。候选仍使用容量 16 的单消费者队列与 Room v11 中由 v6 引入的安装私有 HMAC 观察租约。代码只用脱敏夹具回放，尚未完成当前 5-route catalog 的系统 callback、中文微信正文、更新/重启、OEM 后台和资源真机验收，因此不是支付宝、微信或招商银行的正式支持声明。
- Debug 模板研究入口：开发包另有独立桌面入口，由用户选择精确包名并手动开始、停止和清除。它只追加开始之后的新 callback 到 app-private no-backup 文件，跨进程保持活动，不设自动时间、条数或文件大小限制；不读历史通知、notification key、actions、RemoteViews，也不创建账本记录。页面每次倒序读取最多 10 条，逐项展示来源、时间、channel/category 与五个允许正文域，明确标出未提供字段，并显示当前 listener 状态及本进程可确认的队列/正文漏记；系统从未交付的通知无法反推出内容或精确数量。21 条真实通知样本与 11 张过程截图已按用户要求只保存在两份 Git 忽略的本机私有副本，设备端原始数据也未删除；离线安全盘点把可证明形态收敛为上述 5 个候选规则，其余样本保持拒绝。Release 包没有采样页面、原始文件名、研究包名或研究文案。
- 用户确认对账切片：待复核 Draft 只在关系特定硬门满足时产生有限的转账、信用卡还款、退款或 `FUNDED_BY` 建议；永不只按相同金额自动合并。绑卡支付候选还要求两条外部支出分别审核为支付宝/微信与银行渠道，并严格匹配币种、银行卡/信用卡账户、规范化商户和 30 分钟窗口。用户在竖屏底部面板确认后，应用原子生成一笔平衡的 `TRANSFER`、`LIABILITY_REPAY`、`REFUND` 或替代 `EXPENSE`，并把被吸收 Draft 标为可撤销的 `LINKED`；撤销交易会恢复 Draft 与两条证据链。一般重复、pending/posted 和自动合并仍未实现。
- Room 与界面：schema v11 保存账本、不可变余额快照、RawEvent、ParseAttempt、来源建议、Draft 证据链接与可纠正业务渠道、载荷生命周期/策略、暂存租约、通知观察摘要、导入批次/行结果、对账链接/关系、审计与幂等回执；跨表不一致失败关闭。账户页可为现金、银行、钱包余额和信用卡记录实际余额/欠款，并显示同一时点的账本余额与带符号差异；差异不会自动创建交易。总览、账户、完整草稿编辑、流水、分享复核、导入映射、对账和证据设置均由本地状态驱动。
- 验证：2026-08-01 最终工作树的 `test lint` 成功（571 个 actionable tasks：112 executed、4 from cache、455 up-to-date）；`assembleDebug assembleRelease :data:local:assembleDebugAndroidTest :ocr:paddle:assembleDebugAndroidTest :app:assembleDebugAndroidTest` 成功（711 个 actionable tasks：52 executed、659 up-to-date）。arm64-v8a Release 为 90,787,129 bytes（SHA-256 `1cdb540fc7e9b23203e14182249d97e4566a4fef74c1e31542d9f92094ef47b7`），x86_64 Release 为 128,608,818 bytes（SHA-256 `1b3b44bf1b0e2a25fefa5f88ac00e945ed9ddccae482cb299e7662a65fd740ab`）；两包通过 16 KiB ZIP 对齐。港版 S24 Ultra/API 36 上最终 `data:local` 55/55、app 16/16 通过，最终 arm64 Debug 覆盖安装并冷启动成功；随包 OCR 合成三语、空间 OCR、11 张本机私有真实繁中/英文过程页与简中合成状态页保留既有通过证据。设备轮没有读取通知历史、来源 App、真实截图或本机私有样本；现有 5 条 provider route 的生产系统 callback 仍须发布前验收。完整证据见 [可靠性](docs/RELIABILITY.md)，设备结果与未测项见 [Android 设备兼容与真机验收](docs/design-docs/android-device-compatibility.md)。
- 外部来源：支付宝、微信支付和招商银行现各有范围很窄的实验性通知适配器与脱敏回放，但只覆盖上述 5 种模板；专属文件适配器、完整事件覆盖、真机系统 callback 回放和发布证据仍未完成，因此三类来源当前状态仍统一为 `FALLBACK_REQUIRED`。通用文本/CSV/TSV 入口只证明 Bill 能处理用户明确选择并映射的不可信文件，不证明来源 App。银行卡只覆盖银行卡/信用卡资金腿，不能替代微信零钱或支付宝余额内部流水。
...
普通 `assembleRelease` 仍只生成未签名审计包。小范围测试若只用 ADB，可继续安装 Debug；若要发给多台测试机并在以后覆盖安装保留本地账本，则使用一把长期固定、仓库外保存且单独备份的内部测试 keystore。当前仓库已经完成首次身份创建并提交公开证书指纹；日常从真实本机 CMD 运行，密码隐藏输入且不进入参数、仓库或日志：

```bat
scripts\internal-signing.cmd status
scripts\internal-signing.cmd build
```

`create` 只用于尚未固定证书的新项目首次引导；一旦 `scripts/internal-signing-certificate.sha256` 存在，缺失 keystore 必须从备份恢复，脚本拒绝生成同名新身份。密码最低 6 位、建议至少 12 位。`build` 要求输入递增 `versionCode`、`versionName` 和同一密码，先比对公开证书指纹，再调用单次 Gradle 进程并验证两个 ABI APK。除 Gradle 原始文件外，发行链会在同一输出目录生成并再次验签 `Bill-Local-Ledger-Android-arm64-v8a.apk` 与 `Bill-Local-Ledger-Android-x86_64.apk`；发给 Android 真机的是 arm64-v8a 版本。高级/CI 场景仍可显式设置 `BILL_VERSION_CODE`、`BILL_VERSION_NAME` 和四项 `BILL_RELEASE_*` 后运行 `scripts\release.cmd`。两条路径都不上传 GitHub 或应用商店；密钥或密码缺失、部分配置、仓库内 keystore、身份漂移或验签失败都会失败关闭。
